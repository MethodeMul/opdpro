	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			dragEnd: function () {
				...
				if (this.colresize)
				{
					...
					if (p.cookies){
						...
						prefs['colsize'] = (typeof prefs.colsize == 'undefined') ? {} : prefs.colsize;
						prefs['colsize']['col_'+n] = nw;
						g.prefs.data = prefs;
						g.prefs.save();
					}
				}
			...
			populate: function () { //get latest data
				...
				if(p.cookies){
					...
					if(typeof prefs.page != 'undefined') p.newp = prefs.page;

					if(typeof prefs.sortname != 'undefined' && typeof prefs.sortorder != 'undefined'){
						p.newp = prefs.page;
		...
		if (p.colModel)
		{
			...
			for (i=0;i<p.colModel.length;i++)
			{
				...
				if(p.cookies){
					var prefs = g.prefs.load();
					if(typeof prefs.colsize != 'undefined' && typeof prefs.colsize['col_'+i] != 'undefined') $(th).attr('width',prefs.colsize['col_'+i]);
				}
		...
		function ()
		{
			...
			{
				if(p.colMove) g.dragStart('colMove',e,this);
			})
			.hover(
			function(){
		...
		if (p.usepager)
		{
			...
			if (p.searchitems)
			{
				...
				for (var s = 0; s < sitems.length; s++)
				{
					...
					if(p.cookies) var prefs = g.prefs.load();
				...
				$(g.sDiv).append("<div class='sDiv2'>Quick Search <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='button' id=\"flexi_search\" value='Procurar' />&nbsp;<input type='button' id=\"flexi_reset\" value='Reset' /></div>");
				...
				if(p.cookies){
					...
					if(typeof prefs.q != 'undefined'){
						if($(g.sDiv).not('visible')) $('.pSearch',g.pDiv).click();
					}
				}
